...
def analyze_chunk_quality(text: str) -> dict:
    ...
    return {
        ...
        "corruption_ratio": sum(1 for c in text if ord(c) > 127 and c not in 'áéíóúñÁÉÍÓÚÑüÜ¿¡') / max(len(text), 1)
    }
...
def sample_chunks_per_party(client: QdrantClient, partido: str, sample_size: int) -> list:
    ...
    while len(chunks) < sample_size:
        result = client.scroll(
            ...
            scroll_filter=Filter(
                must=[
                    FieldCondition(key="partido", match=MatchValue(value=partido))
                ]
            ),
            with_payload=True
        )
        ...
        for point in points:
            ...
            if chunk_count % max(1, chunk_count // sample_size) == 0 or len(chunks) < sample_size:
...
def main():
    ...
    if FULL_SCAN:
        ...
        print(f"💡 To scan all chunks, set VERIFY_FULL_SCAN=true\n")
    
    # Get unique parties
    print(f"Discovering parties...")
    ...
    if not FULL_SCAN:
        print(f"   ({SAMPLE_SIZE} samples per party, ~{len(parties) * SAMPLE_SIZE} total)\n")
    else:
        print(f"   (FULL SCAN)\n")
    
    print("=" * 90)
    print(f"{'PARTIDO':<10} {'TOTAL':<8} {'SAMPLED':<10} {'CORRUPTED':<12} {'AVG CORRUPT %':<15} {'STATUS':<10}")
    print("=" * 90)
    ...
    for partido, stats in sorted_parties:
        ...
        avg_corruption = sum(stats["corruption_ratios"]) / len(stats["corruption_ratios"]) * 100 if stats["corruption_ratios"] else 0
        ...
        total_estimate = f"~{total_chunks // len(parties)}" if not FULL_SCAN else str(sampled)
        
        print(f"{partido:<10} {total_estimate:<8} {sampled:<10} {corrupted:<12} {avg_corruption:<15.1f} {status:<10}")
    ...
    if critical_parties:
        ...
        print("\n📋 Recommended actions:")
        print("1. Run FULL SCAN to confirm: VERIFY_FULL_SCAN=true python verify_quality.py")
        ...
        if not FULL_SCAN:
            print("   (Based on sampling - run VERIFY_FULL_SCAN=true for complete verification)")
        return 0
